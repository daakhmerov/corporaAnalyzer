def process_corpora(input_files_dir:str, output_dir:str, log_flow:list, start=None, end=None):
    ...
    all_files = [f"{os.path.join(input_files_dir, file)}" for file in os.listdir(input_files_dir)]
    ...
    try:
        for file in tqdm(all_files):
            if os.path.splitext(file)[-1] == '.pdf':
                ...
                if checking_status == 'warning':
                    process_result = process_newspaper(file, log_flow, output_dir)
        ...
        log_flow.append(LogString('danger', f'Процесс прерван, т.к. не установлен следующий модуль\n⤷{m_e}\n'))
    except Exception as e:
        log_flow.append(LogString('danger', f'Процесс прерван из-за следующей ошибки\n⤷{e}\n'))
