def pipeline(project_subdir: str, log_flow: list, clean_data: bool = True, start: int = None, end: int = None, to_project_dir: bool = False, project_dir: str = None):
    ...
    if to_project_dir is True and project_dir != None:
        corpora_dataset_output = os.path.join(
            project_dir, '.project_datasets_output')
    else:
        corpora_dataset_output = os.path.join(
            project_subdir, 'corpora_dataset_output')
    corpora_tokens_output = os.path.join(project_subdir, 'tokens')

    # Обработка данных
    # Обработка газет
    try:
        process_corpora(project_subdir, corpora_tokens_output,
                        log_flow, start, end)
        ...
        LogString('danger', f'Ошибка обработки корпуса газет\n⤷{e}\n')
